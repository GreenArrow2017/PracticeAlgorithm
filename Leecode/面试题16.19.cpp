//
// Created by GreenArrow on 2020/9/10.
//
#include <iostream>
#include <vector>
#include <algorithm>
...
public:
    ...
    vector<int> pondSizes(vector<vector<int>> &land) {
        vector<int> results;
        int nRow = land.size();
        int nColumn = land[0].size();
        for (int i = 0; i < nRow; i++) {
            for (int j = 0; j < nColumn; j++) {
                if (land[i][j] == 0) {
                    int count = 0;
                    dfs(land, i, j, nRow, nColumn, count);
    ...
    void dfs(vector<vector<int>> &land, int x, int y, int nRow, int nColumn, int &count) {
        land[x][y] = -1;
        count ++;
        for (int i = 0; i < 8; i++) {
            int x_next = x + direction[i][0];
            int y_next = y + direction[i][1];
            if (x_next >= 0 && x_next < nRow && y_next >= 0 && y_next < nColumn && land[x_next][y_next] == 0) {
                dfs(land, x_next, y_next, nRow, nColumn, count);
            }
        }
    }
};
